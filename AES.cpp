...
uint8_t AES::FFAddByte(uint8_t a, uint8_t b) // Adds two finite fields for 8 bits
{
    return a ^ b;
}

uint32_t AES::FFAddWord(uint32_t a, uint32_t b) // Adds two finite fields for 32 bits
{
    return a ^ b;
}

uint8_t AES::FFMultiply(uint8_t a, uint8_t b) // Uses XTime() to multiply a finite field by another finite field
{
    uint8_t prod;
...
void AES::MixColumns() //Calculated in HM 3
{
    for (int col = 0; col < Nb; col++) {
        uint8_t s1 = state[0][col],
                s2 = state[1][col],
...
uint8_t AES::XTime(uint8_t ff) // Multiplies a finite field by x
{
    if (ff & (1 << 7)) {
        return (ff << 1) ^ 0x11b;
    } else {
